...
def merge_file(file_path, summarize_file_path):
    # ファイルが存在したら消去.
    if os.path.exists("{summarize_file_path}/allseed_summarize.csv".format(summarize_file_path=summarize_file_path)):
        os.remove(
            "{summarize_file_path}/allseed_summarize.csv".format(summarize_file_path=summarize_file_path))

    data = []
    # ファイルを開いてデータを読み込み、3つ目の時間の値でソート
    for i in range(10):
        # correct_patchのデータ読みこみ.
        with open("{file_path}/correct_patch{i}.csv".format(file_path=file_path, i=i), 'r') as file:
            log = file.read()
        # Split the log into lines
        correct_lines = log.split('\n')
        log = ""
...
def input_file(file_path, summarize_file_path):
    # ファイルが存在したら消去.
    if os.path.exists("{summarize_file_path}/all_summarize.csv".format(summarize_file_path=summarize_file_path)):
        os.remove(
            "{summarize_file_path}/all_summarize.csv".format(summarize_file_path=summarize_file_path))

    with open("{file_path}/allseed_summarize.csv".format(file_path=file_path), 'r') as file:
        log = file.read()
...
def count_output(file_path, summarize_file_path):
    with open("{file_path}/all_summarize.csv".format(file_path=file_path), 'r') as file:
        log = file.read()
    ...
    df.to_csv(
        "{summarize_file_path}/seed_summarize.csv".format(summarize_file_path=summarize_file_path))
    if os.path.exists("{summarize_file_path}/all_summarize.csv".format(summarize_file_path=summarize_file_path)):
        os.remove(
            "{summarize_file_path}/all_summarize.csv".format(summarize_file_path=summarize_file_path))
