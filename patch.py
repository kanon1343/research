    ...
    for i in range(1, count):
        ...
        # メトリクスの合計を取得.
            ...
            if file_name == df.iat[j+1,7]:
                ...
                dicts = {"nloc":nloc, "ccn":ccn, "token":token, "parameter":parameter, "length":length, "repairLines":repair_lines, "filename":file_name}
...
def main():
    ...
    # p = pathlib.Path("/Users/haradakanon/Desktop/研究/実験データ/research/sample")
        clean(p)
        # analyze_code(p)
        # defects4j = pathlib.Path("./math85/src/java")
        # analyze_element(p)
        # get_point(p, defects4j, i)
